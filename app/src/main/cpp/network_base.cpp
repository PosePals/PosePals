...
void NetworkBase::smoothingLandmarks(std::vector<Keypoint>& detects, int img_w,int img_h)
{
...
int NetworkBase::load(AAssetManager* mgr, const char* modeltype, bool use_gpu)
{
    anchors.clear();
    ...
    pose_detection.load_param(mgr, "nn/detection.param");
    pose_detection.load_model(mgr, "nn/detection.bin");
...
int NetworkBase::draw(cv::Mat& rgb, const std::vector<Object>& objects)
{
